...
*For contributors who do not code. Find your recipe, copy the boxes, send them. The agent does the rest.* 

## How it works
You describe and decide. The agent codes, tests, and reviews. Four parts:
- **Claude Code** — the terminal program you type to. One conversation = one **session**.
- **The skills pack** — the `mattpocock-skills` plugin (by Matt Pocock, from the official plugin list): `/` commands that load a skill, a proven way to do one type of work.
- **The spine** — the file `docs/plans/next-steps.md` in your repo: the live work list and the handoff between sessions.
- **The decision log** — the folder `docs/adr/` in your repo: one small file per settled call, named for its ID (`docs/adr/DEC-GIT-FLOW.md`). The spine holds work; this folder holds answers.
...
- Unchecked ⚡ line = work that waits for an agent.
- Checked ⚡ line = built and tested; its PR (a package of changes) waits for review and ship.
- 🧑 line = waits for **you**: a question, a decision, or an operator-only task.

A settled call never stays in the spine: the agent writes it to `docs/adr/` as a `DEC-*` file and the spine only points at the folder. A decision that still waits for your word keeps a 🧑 line until you give it.

Badges: 🚶 send and walk away · 🧑 stay and answer · 🚶→🧑 runs alone, final call is yours.

## Pick your recipe

| You want to… | Recipe |
|---|---|
| Set up a new machine or repo | 1 · One-time setup 🚶 |
| Add a feature | 2 · New feature 🧑→🚶 |
| Tweak something tiny — a line of text, a label, a color | 2a · Quick change 🚶 |
| Add a feature too foggy to describe | 3 · Map the fog 🚶→🧑 |
| Answer one design question with a test build | 4 · Design detour 🚶→🧑 |
| Build the queued work | 5 · Build 🚶 |
| Review a PR, hunt bugs, harden code | 6 · Review 🚶 |
| Ship finished work | 7 · Ship 🚶→🧑 |
| Sort incoming reports and requests | 8 · Triage 🚶 |
| Fix something broken | 9 · Broken 🚶 |
| Fix a merge stuck half-way | 10 · Merge mess 🚶 |
| Get optimization / clean-up ideas | 11 · Survey 🚶 |

Normal feature journey: **2 → 5 → 6 → 7**. Tiny tweak: **2a → 7**.
...
- The `/words` at a box's start must stay the first words of the message (some never show in the typing menu — they work anyway).
...
When you come back, the session's last message shows one of three states: **goal met** (evidence on screen: green tests, PR URL, spine sections) · **paused** (answer as above) · **one item blocked** (parked under 🧑 with its blocker named; the rest continued).
...
(Or type `/plugin`, open **Discover**, and install **mattpocock-skills**.) Then start a **new** session — skills load at session start. Update later from `/plugin` → **Installed**. First machine? Set permissions once with the maintainer; if something fails, ask them.

**Part B — once per repo.** Skip if `docs/plans/next-steps.md` already exists. New session at the repo root:

✏️
```
/mattpocock-skills:setup-matt-pocock-skills configure GitHub as the Issue tracker, then scaffold the repo spine:
...
📋
```
/goal the chore/bootstrap PR is open — URL shown in this session — with docs/plans/next-steps.md (🔧 seeded), docs/adr/DEC-GIT-FLOW.md, and the three CLAUDE.md standing blocks in its diff; OR paused on a DEC-worthy fork — options and a recommendation shown, still unanswered by me
```
...
### 2. New feature 🧑 then 🚶
**When:** the product must do something new, or the change needs real decisions. This is the start of the journey — **stay** for the interview. *(Just a tiny tweak? Use 2a.)*

✏️ Send, then answer the agent's questions, one by one, until it has none left:
```
/mattpocock-skills:grill-with-docs /mattpocock-skills:codebase-design grill me on the idea below — record hard calls as DEC-* files in docs/adr/, one file per decision, as they settle. The idea:
...
```
/mattpocock-skills:to-spec spec everything agreed above — pin the test seams: existing seams preferred, at most one new; a seam you had to invent gets an OPEN marker in the spec plus a 🧑 line, not a question to me
```

```
/mattpocock-skills:to-tickets cut the spec into agent-sized Issues and seed each as a one-line ⚡ item linking its Issue in @docs/plans/next-steps.md; an Issue depending on an OPEN seam or any 🧑-parked question → 🧑 with it named, never ⚡, until I ratify it as a DEC-* file in docs/adr/
```

```
/goal the spec file exists and is linked from @docs/plans/next-steps.md, at least one Issue created with every Issue's ID or URL shown in this session, and the ⚡/🧑 sections shown after the last edit holding one line per Issue (OPEN-seam and open-question Issues under 🧑); OR paused on a DEC-worthy fork — options and a recommendation shown, the fork filed as a 🧑 line and still unanswered by me
```

**Result:** your idea is now Issues (tracked units of work on GitHub), one ⚡ line each. **Next →** Build (recipe 5).

### 2a. Quick change 🚶
**When:** you can say the whole change in one or two sentences, and it does not change how the product behaves — a line of text, a label, a color, a link, an image. In doubt? Use recipe 2: a tiny idea just makes a short interview. And if the agent finds the "tweak" is bigger than it looks, it pauses and tells you.

✏️ + 📋:
```
/mattpocock-skills:tdd /mattpocock-skills:implement make the small change below — do 🔧 first; work on its own branch per this repo's established git hygiene, its PR opened without asking, and log it as one checked-off ⚡ line linking the PR in @docs/plans/next-steps.md; add a regression test only where the testing discipline warrants one — a pure copy/styling tweak warrants none; the change needs a product call, a new seam, or outgrows one small slice → pause: options + a recommendation (say if the New feature recipe is the right route), filed as a 🧑 line, wait for me; blocked → park it under 🧑 with the blocker named. The change:
...
The map splits the epic into regions; a **frontier** is the next unexplored one. To explore it: new session, 📋 the line below, **stay** (no `/goal` here):

```
/mattpocock-skills:wayfinder /mattpocock-skills:grilling /mattpocock-skills:domain-modeling work the map with me — the next frontier is named under 🧑 in docs/plans/next-steps.md; when a call settles, write it as its own DEC-* file in docs/adr/ named for the decision, not a sequential number and not a line in the spine
```
...
✏️ + 📋:
```
/mattpocock-skills:prototype build a throwaway that settles the design question below — run it, and write the verdict as its own DEC-* file in docs/adr/ — named for the decision, frontmatter "status: proposed — operator confirms", linking the prototype dir and its run command — plus a 🧑 line in docs/plans/next-steps.md naming that file so I see it waits for me; a DEC-worthy fork outside the question → pause: options + a recommendation, filed as a 🧑 line, wait for me. The question:
<the decision it must settle and what evidence settles it>
```

```
/goal the prototype ran — command and output (or screenshot) shown in this session — the proposed DEC-* file shown written under docs/adr/, and its 🧑 line shown added to docs/plans/next-steps.md; OR paused on a DEC-worthy fork — options and a recommendation shown, the fork filed as a 🧑 line and still unanswered by me
```
...
**When you return:** the verdict is only *proposed*. Look at the prototype (you can type: "run the prototype for me and show me the output"), then tell the agent to accept the DEC-* file — its status becomes accepted and its 🧑 line goes — or to delete it.

**Question appeared mid-interview (recipe 2)?** Park it there with ✏️ `/mattpocock-skills:handoff design detour: <the question>` → run this recipe in a new session → open one more session and type `/mattpocock-skills:handoff` alone to resume the interview.

### 5. Build 🚶
**When:** the ⚡ queue holds Issues (after recipe 2). Tests first, then code, one PR per item.

**Variant A — drain the whole queue** (short queue). 📋 both:
```
/mattpocock-skills:tdd /mattpocock-skills:implement drain the ⚡ queue in @docs/plans/next-steps.md following this repo's established git hygiene; a blocked item → 🧑 with the blocker named, then continue; a DEC-worthy fork (hard to reverse / product intent / new seam) → pause: options + a recommendation, filed as a 🧑 line, wait for me; keep the spine current as you go
```
...
**Variant B — one item per session** (long queue; repeat in fresh sessions, the spine hands over between them). 📋 both:
```
...
*(Previews: send `/vercel:deploy` — or the maintainer's `/wrangler` command — as its own message between the two boxes.)*

**The agent never releases to production alone.** A production release always lands as a 🧑 line: you run it, with the maintainer if needed.

### 8. Triage — sort incoming reports 🚶
**When:** bug reports or requests arrived from other people (never recipe 2's own output). Work lands in ⚡; judgment calls land in 🧑 for you.

✏️ + 📋:
```
/mattpocock-skills:triage triage the batch below — raw issues I didn't write only, never /mattpocock-skills:to-tickets output; write an agent-ready brief per issue landing as one ⚡ line linking its issue in @docs/plans/next-steps.md; close/priority/product-intent calls → 🧑 with the reason, never decided solo; a fork blocking the whole batch → pause: options + a recommendation, filed as a 🧑 line, wait for me. The batch:
...
<paste the exact error or failure output, and where it bites>
```
...
*(Bug only appears on your machine or account? The agent cannot see it — stay and run what it asks.)*
...
**When:** spare agent time. Report only — it changes no code.

✏️ + 📋:
```
/mattpocock-skills:improve-codebase-architecture /mattpocock-skills:codebase-design survey <the whole repo, or one subsystem> for the highest-leverage structural improvements — show git status before touching anything; REPORT ONLY: each pick lands as one 🧑 idea line (what, why, entry file) flagged "grill before building" in @docs/plans/next-steps.md — ideas are operator work, never ⚡; change no source code
```
...
- **Wrong recipe, or session grew long:** ✏️ `/mattpocock-skills:handoff <what the next session must do>` → open a new session → start the right recipe there (`/mattpocock-skills:handoff` alone resumes the saved work).
...
`git log`, not here. Settled decisions live in `docs/adr/`, not here.
...
- **Decisions → `docs/adr/`** — one `DEC-*.md` file per hard-to-reverse call, do-not-reopen. Read that folder before you re-open a settled call; a new decision gets a new file there, never a line here.
...
the file name; never renumber, never batch two decisions into one file. Frontmatter only when the
status matters — `proposed` until the operator accepts it (recipe 4), then `accepted`.
...
- Read the spine before starting any work. The ⚡ queue is the only source of agent-ready work; 🧑 items are operator-only.
- Keep it current AS you work, not at the end: check off / delete what ships (git log is the record), add newly surfaced work as one-line ⚡ items linking to their issue/PR/spec, record new traps as lessons.
- Hard-to-reverse decisions never live here: each one is its own `docs/adr/DEC-<SLUG>.md` — title, then 1-3 sentences of context, decision, and why. Read `docs/adr/` before you re-open a settled call; the spine only points at the folder. A decision still waiting on the operator keeps `status: proposed` plus a 🧑 line naming its file.
