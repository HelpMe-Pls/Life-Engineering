...
- "_What was the primary business driver or technical limitation that necessitated building this particular EDA (event-driven architecture)?_"
	- The primary driver was a critical **technical limitation** causing significant **business impact**:
		...
		- The underlying reason is that _==the total duration==_ required to complete the _==entire transaction flow for one request==_ tied up the main thread for too long, preventing event loop from rapidly cycling through and processing other pending events or handling new incoming requests efficiently.
...
- "_How was the RabbitMQ cluster itself **configured and operated** to ensure high **availability and eventual consistency** for the application?_"
	- We agreed on these configs and conventions for our worker service:
		...
		- ==**Manual Acknowledgements**==: to prevent message loss if the worker crashes mid-processing, I made sure that a message was _only_ removed from the queue after my service successfully processed it (via `channel.ack(msg)`) or _==explicitly rejected/dead-lettered==_ it via `channel.nack(msg, false, requeue=false)`.
