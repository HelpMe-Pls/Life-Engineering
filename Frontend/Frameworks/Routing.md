...
- If you want a child pathname segment to have its UI separated to a new page (i.e. its layout is decoupled from the UI of its *direct* parent pathname), add an underscore (`_`) to its parent. For example, the `users/random/notes` path will have the UI of `notes.tsx` decoupled from `users` if we define the route as `routes/users+/random_+/notes.tsx`. In this case, `users/random` and `users/random/note` pathnames are treated as siblings.
...
# Links
- Use the `<Links/>` component to dynamically access the HTML `<link/>` tags from anywhere in your project (that being said, in those files that don't have access to the `<head/>` tag). You can also get your resources dynamically cached on the browser:
```tsx
