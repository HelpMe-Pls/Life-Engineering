...
```js
// Will this code print `0` (sync callback invocation) or `1` (async callback invocation)? That's out of our control
...
> [!important]- The callback's timer represents the delay until the said callback is pushed into to [[Async#Event Loop |macrotask queue]], **not** into the callstack.
...
> [[Promise]] solves the "inversion of control" in callbacks.