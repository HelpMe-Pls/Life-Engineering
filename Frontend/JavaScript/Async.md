...
- In other words, your program is generally broken up into lots of small chunks, which are executed one after the other, and that order is managed by the event loop. And technically, other events not related directly to your program can be [[NodeJS#Threads and Processes |interleaved]] within the callstack as well.
