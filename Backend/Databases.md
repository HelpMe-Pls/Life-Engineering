...
```
### One-to-many
- The differences are at the relation field's type on the "One" model and the relationship attribute's type on the "Many" model:
```cs
model Person {
  id           String        @unique @default(cuid())
  name         String
...
// On production:
// 1. Backup
// 2. Create a new database file (locally, let's call it `seed.db`, generated by running `npx prisma migrate deploy`)
